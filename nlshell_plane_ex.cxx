...
#include "shellmechanics/operators/NonlinearShellWeakForms.h"
...
static const std::string simulation_name = "nlshell3d";
...
/////////////////////////////////////////////////////////////////////////////////////
*/
void aplane(double L, NurbsSurface &surface)
{
  surface.p = 1;
  surface.q = 1;
...
{
  // get the geometry
  double const L = 1.0; double const thickness = 0.01;
  NurbsSurface shape; aplane(L,shape);
  spline_ops::elevate(3,0,shape);
  spline_ops::elevate(0,1,shape);
  // spline_ops::midpointRefinement(3,0,shape);
  // the material
  double const E = 70e9; double const nu = 0.2;
  ...
  {
    // add the weak forms
    using material_form = NonlinearMaterialStiffness<MembraneMaterialTangent,TransverseShearMaterialTangent>;
    using geometric_form = NonlinearGeometricStiffness<MembraneMaterialTangent,TransverseShearMaterialTangent>;
    auto &wfm = WeakFormManager::instance();
    wfm.add<material_form>(material_form(matl_m, matl_t, thickness));
    wfm.add<geometric_form>(geometric_form(matl_m, matl_t, thickness));
    // residual
    using residual_form = NonlinearMaterialResidual<MembraneMaterialTangent,TransverseShearMaterialTangent>;
  ...
  NurbsCurve fixed; spline_ops::getIsoCurve(0,0,shape,fixed);
  {
    auto &mgr = ConstraintManager::instance();
    mgr.add<FixedFixed>(fixed,FixedFixed());
  }

  NurbsSurface load = shape;
  {
    auto &mgr = BoundaryConditionManager::instance();
    using load_T = AppliedPressure<RampedValue<double>>;
    mgr.add<load_T>(load, 25e3, 1.0, 0);;
  }
  ...
  // solve
  {
    ...
    size_t const inner_steps{100};
    size_t const load_increments{100};
