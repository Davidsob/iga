...
static std::string const simulation_name = "shell3d-cylinder";
...
{
  // get the geometry
  double const R = 300.0; double const L = 300.0; double const thickness = 3.0;
  NurbsSurface shape; acylinder(R,L,shape);
  spline_ops::elevate(2,0,shape);
  spline_ops::elevate(3,1,shape);
  // spline_ops::midpointRefinement(2,0,shape);
  ...
  // the material
  double const E = 3e6; double const nu = 0.3;
  ...
  NurbsCurve xsymm,ysymm,zsymm;
  spline_ops::getIsoCurve(0.0,0,shape,xsymm);
  spline_ops::getIsoCurve(0.0,1,shape,ysymm);
  spline_ops::getIsoCurve(1.0,0,shape,zsymm);
  EdgeNormalSource nx({-1,0,0});
  EdgeNormalSource ny({0,-1,0});
  EdgeNormalSource nz({0,0,-1});
  // spline_ops::writeToFile(ysymm,geometryFile,20);
  // showCurve(ysymm, geometryFile);
  {
    auto &mgr = ConstraintManager::instance();
    mgr.add<RotationSymmetryConstraint>(xsymm,0.0,0,nx);
    mgr.add<RotationSymmetryConstraint>(zsymm,1.0,0,nz);
    mgr.add<RotationSymmetryConstraint>(ysymm,0.0,1,ny);
  }

  // point constraints
  // get all points on some planes
  GeometryUtils::Plane<3> plane({0,0,0},{-1,0,0});
  auto const onx = pointsOnPlane(plane);
  plane.n = {0,-1,0};
  auto const ony = pointsOnPlane(plane);
  plane.n = {0,0,-1};
  auto const onz = pointsOnPlane(plane);
  plane.x = {0,L,0};
  plane.n = {0,1,0};
  auto const ony2 = pointsOnPlane(plane);
  {
    using U = PrescribedShellDisplacementU;
    using V = PrescribedShellDisplacementV;
    using W = PrescribedShellDisplacementW;
    // add a bunch of point constraints
    auto &mgr = ConstraintManager::instance();
    for (auto const &p : onx) mgr.add<U>(p,0.0);
    for (auto const &p : ony) mgr.add<V>(p,0.0);
    for (auto const &p : onz) mgr.add<W>(p,0.0);
    for (auto const &p : ony2) mgr.add<U>(p,0.0);
    for (auto const &p : ony2) mgr.add<V>(p,0.0);
    for (auto const &p : ony2) mgr.add<W>(p,0.0);
  }

  BSplinePoint load;
  load.Q.push_back(spline_ops::SurfacePoint(0,0,shape));
  {
    using value_t = StaticVectorR<3>;
    auto &mgr = BoundaryConditionManager::instance();
    using load_T = PointLoad<ConstantValue<value_t>>;
    // using load_T = AppliedPressure<RampedValue<double>>;
    mgr.add<load_T>(load,value_t({0,0,-0.25*1e6}));
  }
  ...
  // solve
  {
    ...
    // solver.run();
    using results_t = Results<NurbsSurface,SurfaceWriter>;
    results_t results(shape, SurfaceWriter(20,20), simulation_name);
    results.write();
  }
