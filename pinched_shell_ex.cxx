...
void run(Surface const &surf)
{
    ...
    &constraint,h](auto const &p)->Eigen::MatrixXd
  {
    ...
    return h*(km+kt) + std::pow(h,3)/12.0*kb + h*kc;
    // return h*(km+kt) + std::pow(h,3)/12.0*kb;
  };
  ...
  // integrate
  {
    ...
    auto ip  = iga::integrationPoints(mapper,surf.p-1,surf.q-1);
  ...
  // apply point load
  {
    GeometricDofManager gtmp = geom;
    BSplinePoint p;
    p.Q.push_back(spline_ops::SurfacePoint(0,0,surf));
    gtmp.addShape(&p);
    for (auto const &idx : gtmp.idsForShape(&p))
    {
      f(ndof*idx+2) = -0.25;
    }
  }
  ...
  {
    ManifoldElementMapper mmap(surf);
    auto normal = [&mmap](auto const u, auto const v)->Vector3d
    {
      return mmap.normal(std::array<double,2>({u,v}));
    };

    auto rot_symmetry = [rid,dot](auto const &p, auto const &dir, auto const &uv, auto const &normal)->MatrixXd
    {
      auto const mapper = dynamic_cast<CurveElementMapper const *>(&p.mapper);
      auto const shape = p.mapper.shape(p.para);
      Vector3d const t1 = mapper->tangent(p.para);
      Vector3d const n = !dir ? normal(uv,p.para[0]) : normal(p.para[0],uv);
      Vector3d const t2 = n.cross(t1);
      return dot(t2,rid(shape));
    };

    // curve at u = 0
    // a pair of dir,coord for an isocurve
    std::vector<std::pair<size_t, double>> isoPairs;
    isoPairs.push_back(std::make_pair(0,0.0));
    isoPairs.push_back(std::make_pair(0,1.0));
    isoPairs.push_back(std::make_pair(1,0.0));

    size_t ii = 0;
    for (auto const &isoPair : isoPairs)
    {
      auto const dir = isoPair.first;
      auto const uv = isoPair.second;
      NurbsCurve curve; spline_ops::getIsoCurve(uv,dir,surf,curve);
      std::cout << curve << std::endl;

      CurveElementMapper cmapper(curve);
      auto ip = iga::integrationPoints(cmapper,curve.p);
      ...
      Eigen::MatrixXd Ksymm(1,edof); Ksymm.setZero();

      // get dofs
      for (size_t i = 0; i < elu; i++)
      {
        std::printf("\nIntegrating boundary element(%lu)\n",i);
        cmapper.updateElementMesh(i);
        for (auto &p : ip) p.update();
        quadrature::gauss(ip,Ksymm,rot_symmetry,dir,uv,normal);
      }

      GeometricDofManager gtmp = geom;
      gtmp.addShape(&curve);
      std::vector<size_t> sdof; sdof.reserve(edof);
      for (auto const idx: gtmp.idsForShape(&curve))
      {
        for (size_t j = 0; j < ndof; j++) sdof.push_back(ndof*idx+j);
      }

      size_t j = 0;
      for (auto &dof : sdof)
      {
        if (!algo::equal(Ksymm(0,j),0.0))
          constraints.push_back(triplet(ii,dof,Ksymm(0,j)));
        j++;
      }
  ...
  // apply fixed conditions
  {
    ...
    {
      constraints.push_back(triplet(cdof++,ndof*idx,1.0));
    }
    ...
    for (auto const &idx : pointsOnPlane(geom,plane))
    {
      constraints.push_back(triplet(cdof++,ndof*idx+1,1.0));
    }
    ...
    for (auto const &idx : pointsOnPlane(geom,plane))
    {
      constraints.push_back(triplet(cdof++,ndof*idx+2,1.0));
    }
  }
...
// #################################
  ...
  double scale = mx > 0 ? 100.0/mx : 1.0;
  // double scale = 1.0;
