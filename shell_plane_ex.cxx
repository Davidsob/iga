...
static const std::string simulation_name = "shell3d";
...
void bplane(double L, NurbsSurface &surface)
{
  double const sq2{std::sqrt(2.0)};
  surface.p = 1;
  surface.q = 1;
  surface.uknot = {0,0,1,1};
  surface.vknot = {0,0,1,1};
  surface.Q = {{0,0,0},{L,0,0}, {0,L/sq2,L/sq2}, {L,L/sq2,L/sq2}};
  surface.weights = {1,1,1,1};
}
...
void sim()
{
  ...
  NurbsSurface shape; bplane(L,shape);
  ...
  NurbsCurve fixed; spline_ops::getIsoCurve(0,1,shape,fixed);
  {
  ...
  // solve
  {
    ...
    size_t const load_increments{1};
