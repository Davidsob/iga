...
void bline(NurbsCurve &curve, double L = 1.0)
{
  curve.p = 1;
  curve.knot = {0,0,1,1};
  curve.Q = {{0,0},{0,L}};
...
void run(Curve const &curve)
{
  ...
  auto Dmat = [E,nu,I,A,G](auto const &p)->Matrix3d
  {
  ...
  auto transformation = [](auto const &p, size_t ndof)->Eigen::SparseMatrix<double>
  {
    ...
    R.block(0,0,2,2) = mapper->localTransformation(p.para);
    R(2,2) = 1.0;
    ...
    {
      th.block(3*i,3*i,3,3) = R;
    }

    return th.sparseView();
  };

  auto stiffness = [&B,&Dmat,&transformation](auto const &p)->Eigen::MatrixXd
  {
    auto const d = Dmat(p);
    auto const b = B(p);

    auto kel = b.transpose()*(d*b);
    auto const R = transformation(p,kel.rows());
    // return kel;
    return R.transpose()*kel*R;
  };

  // Eigen::Vector3d force; force << 0.0,-1,0.0;
  ...
  std::vector<size_t> cdof;
  {
    ...
    {
      cdof.push_back(ndof*idx); // fix u
      cdof.push_back(ndof*idx+1); // fix w
      cdof.push_back(ndof*idx+2); // fix w
    }
...
// #################################
  ...
  std::cout << "v    = " << range(vdisp) << std::endl;
  // plot deformed beam
  show(deformed);
