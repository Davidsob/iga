...
// #################################
...

  {
    ...
    std::cout << "Integrating Volume Integrals" << std::endl;
...
// #################################
  double papp = E/3.0;
  {
    ...
    NurbsCurve curve; spline_ops::getIsoCurve(0.0,1,solid,curve);
    geom.addShape(&curve);
    ...
    std::cout << "Integrating Boundary Integrals" << std::endl;
...
// #################################
  ...
  plane.n = {-1,0};
  // x- plane
  auto const xsymm = pointsOnPlane<Solid>(geom,plane);
  ...
  plane.n = {0,-1};
  auto const ysymm = pointsOnPlane<Solid>(geom,plane);
...
// #################################
  ...
  NurbsSurface wsolution; IO::geometryWithSolution(solid,umag,wsolution);
  size_t N = 100;
