...
namespace spline_ops
{
  ...
  void _elevateU(int t, BSplineSurface &shape)
  {
    ...
    // loop over rows of const v
    for (size_t jj = 0; jj < nv; jj++)
    {
      Uh.clear();
      int const nQ = Qw.size();

      auto mh = ph;
      auto ua = U[0];
      for (int i = 0; i <= ph; i++) Uh.push_back(ua);
      auto kind = ph+1;
      Qw.push_back(Pw[shape.qid(0,jj)]);
      auto cind = 1;

      // initialize first bez segment 
      int lbz = 1; int r = -1;
      for (int i = 0; i <= p; i++) bpts[i] = Pw[shape.qid(i,jj)];
      auto a = p; auto b = p+1;
      while(b < m) // big loop through knot
      {
        auto i = b;
        ...
        auto RBZ = (r > 0) ? rbz+1 : rbz;  // degree elevate bez shape
        for (i = lbz; i <= RBZ; i++)
        {
          ebpts[i] = zero;
          auto mpi = std::min(p,i);
        ...
        if (oldr > 1) // begin removing knot u = U[a]
        {
          ...
          for (int tr = 1; tr < oldr; tr++)
          {
            ...
            while (j-1 > tr) // compute new control points for one removal step
            {
              ...
              {
                auto alf = (ua-Uh[i])/(ub-Uh[i]);
                Qw[i+nQ] = (Qw[i+nQ] - (1.0-alf)*Qw[i-1+nQ])/alf;
              }
  ...
  void elevate(size_t t, size_t direction, BSplineSurface &surf)
  {
    ...
    // else                _elevateV(t,direction,surf);
  }
